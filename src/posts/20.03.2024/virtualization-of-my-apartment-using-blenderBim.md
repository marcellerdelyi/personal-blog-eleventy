...
For the floor plan, there is not much left to say. You click the button, and Bonsai spits out an SVG that you can use right away. For my floor plan, I only added the room dimensions manually, while the walls, windows, door and swing openings were automatically generated by Bonsai.

One thing worth mentioning is that you need to have the **Freestyle SVG Exporter** add-on installed. Otherwise, Bonsai will not let you output an SVG, and the button will stay greyed out. I may or may not have spent a few hours debugging this once, so I wanted to mention it in case you run into something similar.

![Kvennavikgata Floor Plan](../PLAN_VIEW.svg)

Beyond adding the room dimensions, I was also thinking about adding annotations and maybe furniture, both to the 3D model and to the drawings. In the end, it felt a bit unnecessary for now and goes beyond the scope of this article, so maybe that is something for the future.
